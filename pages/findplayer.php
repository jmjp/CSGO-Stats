...
<body class="bg-main">
    ...
    <div class="container bg-linear text-white mt-3 mb-3 rounded container-shadow">
          <div class="col-sm centered text-center mt-3 shadow-lg" style="font-family: 'Titillium Web', sans-serif;">
            <h3 class="pt-3">Find Player!</h3><br>
            <h6>Note: the player needs set the perfil privacy to public for this tool works.</h6>
            </div>
            <div class="container mt-3">
                <form action="/core/steamurlconverter.php" id="search" method="GET">
                    <div class="input-group"><input id="input-query" type="text" name="name" placeholder="Enter a Steam profile url..." class="form-control bg-dark border-dark text-white"><span class="input-group-btn"><button type="submit" form="search" id="search-bar-button" class="btn btn-primary">Search</button></span></div>
                </form>
            </div>
            <div>
                <?php $id = $_GET['id'];
                  if($id && $id != "invalid" && $id != "notpublic"){
                  $playerinfo = getinfo($id);
                  echo '<table class="table align-middle text-center bg-darkness table-bordered table-dark mt-1">
                        <h4 class="text-center mt-3"> <img src="'.$playerinfo->photo.'" class="rounded-circle" height="70" width="70" style="border: 5px solid white;"> '.$playerinfo->name.'</h4>
                        <thead>
                            <tr>
                        ...
                        <tr>
                        <td>playtime</td>
                        <td>'.$playerinfo->playtime.' Hours</td> 
                        </tr>
                         <tr>
                        <td>Kills</td>
                        <td>'.$playerinfo->kills.'</td> 
                         ...
                         <tr>
                        <td>Deaths</td>
                        <td>'.$playerinfo->death.'</td> 
                        </tr>
                         <tr>
                        <td>Win ratio</td>
                        <td>'.$playerinfo->ratio.'%</td> 
                        </tr>
                         <tr>
                        <td>HS percent</td>
                        <td>'.$playerinfo->hspercent.'%</td> 
                        </tr>
                         <tr>
                        <td>Accuracy</td>
                        <td>'.$playerinfo->accuracy.'%</td> 
                        </tr>
                         <tr>
                        <td>Kd</td>
                        <td>'.$playerinfo->kd.'%</td> 
